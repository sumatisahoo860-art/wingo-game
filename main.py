...
<head>
    ...
    <style>
        ...
        .app-header { background: linear-gradient(135deg, #f1c40f, #f39c12); padding: 15px; text-align: center; color: white; font-weight: bold; font-size: 24px; }
        ...
        .n0 { border-color: #bb33ff; background: linear-gradient(135deg, #ff3333 50%, #bb33ff 50%); color: white; }
        .n5 { border-color: #bb33ff; background: linear-gradient(135deg, #00b300 50%, #bb33ff 50%); color: white; }
        .n1, .n3, .n7, .n9 { border-color: #00b300; color: #00b300; }
        .n2, .n4, .n6, .n8 { border-color: #ff3333; color: #ff3333; }
        .multiplier-row { display: flex; justify-content: space-between; margin: 15px; background: #f9f9f9; padding: 8px; border-radius: 8px; gap: 5px; }
...
<script>
    ...
    async function syncGame() {
        try {
            ...
            document.getElementById('game-id').innerText = "202608291000" + data.round_id;
            
            if (lastRoundId !== data.round_id) {
                if (lastRoundId !== null && currentSelection !== null) {
                    let isWin = false;
                    if (currentSelection === data.winning_number.toString()) {
                        isWin = true;
